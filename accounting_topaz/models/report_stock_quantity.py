...
WITH
    existing_sm (id, product_id, tmpl_id, product_qty, date, state, company_id, whs_id, whd_id,z_package_barcode) AS (
        SELECT m.id, m.product_id, pt.id, m.product_qty, m.date, m.state, m.company_id, whs.id, whd.id,pt.z_package_barcode
        FROM stock_move m
    ...
    all_sm (id, product_id, tmpl_id, product_qty, date, state, company_id, whs_id, whd_id,z_package_barcode) AS (
